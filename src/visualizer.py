...
def plot_latent_space(path, z, labels=None, xg_mesh=None, yg_mesh=None, sigma_vector=None, n_points_axis=None):
    plt.figure()
...
def plot_latent_space_ood(path, z_mu, z_sigma, labels, ood_z_mu, ood_z_sigma, ood_labels):
    ...
    for i, (z_mu_i, z_sigma_i) in enumerate(zip(z_mu, z_sigma)):
        ...
        ellipse = Ellipse((z_mu_i[0], z_mu_i[1]), width=z_sigma_i[0], height=z_sigma_i[1], fill=False, edgecolor='blue')
    ...
    for i, (z_mu_i, z_sigma_i) in enumerate(zip(ood_z_mu, ood_z_sigma)):
        ...
        ellipse = Ellipse((z_mu_i[0], z_mu_i[1]), width=z_sigma_i[0], height=z_sigma_i[1], fill=False, edgecolor='red')
        ax.add_patch(ellipse)
