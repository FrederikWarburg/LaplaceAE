...
from visualizer import plot_mnist_reconstructions, plot_latent_space, plot_ood_distributions
...
def test_ae(config):
    ...
    # forward eval
    x, z, x_rec_mu, x_rec_sigma, labels = inference_on_dataset(encoder, mu_decoder, var_decoder, val_loader, device)

    ood_x, ood_z, ood_x_rec_mu, ood_x_rec_sigma, ood_labels = inference_on_dataset(encoder, mu_decoder, var_decoder, ood_val_loader, device)
    
    xg_mesh, yg_mesh, sigma_vector, n_points_axis = None, None, None, None
    if config['use_var_decoder']:
        xg_mesh, yg_mesh, sigma_vector, n_points_axis = inference_on_latent_grid(mu_decoder, var_decoder, z, device)
