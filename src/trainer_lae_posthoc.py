        ...
        with torch.no_grad():
            ...
            if encoder is None:
                mu_rec, var_rec, mu_latent, var_latent = la(
                    X, pred_type=pred_type, return_latent_representation=True
                )
                z_sigma += [var_latent.sqrt()]
            else:
                mu_latent = encoder(X)
                mu_rec, var_rec = la(
                    mu_latent, pred_type=pred_type, return_latent_representation=False
                )
    ...
    for z_grid in tqdm(z_grid_loader):
        ...
        with torch.inference_mode():
            ...
            if encoder is None:
                ...
                f_mu, f_var = la(
                    z_grid, pred_type=pred_type, return_latent_representation=False
                )
