...
class Encoder_stochman_fashionmnist_conv(torch.nn.Module):
    def __init__(self, latent_size, dropout):
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(1, 16, 3, stride=1, padding=1, bias=None),
            nn.Tanh(),
            nn.Conv2d(16, 32, 3, stride=1, padding=1, bias=None),
            nn.MaxPool2d(2),
            nn.Tanh(),
            nn.Conv2d(32, 64, 3, stride=1, padding=1, bias=None),
            nn.Tanh(),
            nn.Conv2d(64, 64, 3, stride=1, padding=1, bias=None),
            nn.MaxPool2d(2),
            nn.Tanh(),
            nn.Conv2d(64, 64, 3, stride=1, padding=1, bias=None),
            nn.Tanh(),
...
class Decoder_stochman_fashionmnist_conv(torch.nn.Module):
    def __init__(self, latent_size, dropout):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Conv2d(64, 64, 3, stride=1, padding=1, bias=None),
            nn.Tanh(),
            nn.Conv2d(64, 32, 3, stride=1, padding=1, bias=None),
            nn.Upsample(scale_factor=2, mode="nearest"),
            nn.Tanh(),
            nn.Conv2d(32, 16, 3, stride=1, padding=1, bias=None),
            nn.Tanh(),
            nn.Conv2d(16, 1, 3, stride=1, padding=1, bias=None),
        )
