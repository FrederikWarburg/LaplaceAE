...
def get_model(number_of_layers, device):
    ...
    model += [nn.Tanh(), nn.Linear(data_size, classes * data_size)]
    model += [nn.Unflatten(1, (classes, data_size))]  # B,C,H,W => B, Classes, C, H, W
...
def get_model_stochman(number_of_layers, device):
    ...
    model += [nnj.Tanh(), nnj.Linear(data_size, classes * data_size)]
    model += [nnj.Reshape(classes, data_size)]  # B,C,H,W => B, Classes, C, H, W
    model = nn.Sequential(*model).to(device)
...
def run_la(data_size, number_of_layers):
    ...
    y = torch.randint(0,classes,(num_observations, classes, data_size)).float()#.long()
...
def run_backpack(data_size, number_of_layers):
    ...
    t0 = time.perf_counter()
    Hs_backpack = bp.CrossEntropyHessianCalculator(model).compute(dataloader)
    elapsed_la = time.perf_counter() - t0
...
def run(data_size, number_of_layers):
    ...
    torch.testing.assert_close(
        Hs_bp, Hs_layer, rtol=1e-3, atol=0.0
    )  # Less than 0.01% off
