...
do
    echo $i
    CUDA_VISIBLE_DEVICES=$device python trainer_lae_elbo.py --config ../configs/mnist_model_selection/lae_elbo_linear.yaml --version $i &
done
CUDA_VISIBLE_DEVICES=$device python trainer_lae_elbo.py --config ../configs/mnist_model_selection/lae_elbo_linear.yaml --version $((i+1));
